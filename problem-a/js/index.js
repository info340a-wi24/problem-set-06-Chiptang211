...
*/
function renderPaletteRow(colors, parent) {
  const containerDiv = document.createElement('div');
  const parentWidth = getElementWidth(parent);
  const boxSize = parentWidth / colors.length;

  colors.forEach(color => {
    const colorBox = createColorBox(color, boxSize);
...
function renderPaletteTable() {
  ...
  for (const key in COLORS_9) {
    renderPaletteRow(COLORS_9[key], mainElement);
  }
}
renderPaletteTable();
